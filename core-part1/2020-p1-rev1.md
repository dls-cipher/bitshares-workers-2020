...
The intent of this contract is to provide funding for development, testing, and deployment of clearly defined BitShares Improvement Proposals (BSIPs).   The targeted BSIPs will provide upgrades to both testnet and mainnet of the BitShares Core protocol.    Our expectation is to complete development of the targeted BSIPs within 6 months and at 1/3rd the cost of the previous Core development team worker.   Upon completion of each BSIP, the worker provides for funding to update corresponding documentation.  This will include changes to User, Dev, GitHub, and Wiki documentation.    To keep our commitments, and provide consistency in the quality of code and other deliverables, this worker will be treated as a corporate agreement, with the blockchain, performed by specific individuals working as a team:  We will not outsource work to 3rd party contractors or through bounties.  
...
As two main consensus affecting releases per year are too much to be expected from a small-sized team, we propose to instead introduce:  

- 1 Testnet Release per year (aim: 15th of July) `test-5.0.0`
*requires consensus update*
- 1 Mainnet Release per year (aim: 10th of August) `5.0.0`
*requires consensus update*
...
**Hardcap definition:**
This Worker reintroduces payouts in BTS, instead of fiat equivalent BitAssets. As such, in the case of appreciation in value of the BTS token over time, this worker will cap employee pay at the value of 0.05 USD per 1 BTS token. The worker begins at lower core rate for hours in concessions to the depreciation of the BTS token value.  BTS payouts are considered fixed (per table 1) as long as the price is <0.05 USD per 1 BTS token. In case of the price >0.05 USD, amount of BTS used for payout will be capped such that:  Maximum USD = BTS rate of this worker * 0.05.

In this manner, our developers can begin development with value oriented low hourly USD rates.   At the same time, they will be rewarded up to cap limit in USD value, if appreciation of BTS:USD occurs during the worker’s duration due to their diligence.  

**Bottom Side Exit Clause**

If at any point the price of BTS:USD is beneath $0.015, each member of the team individually reserves the right to cease their contribution to development, process their outstanding invoices to for all work to date, and exit the contract without any further constraints.   This exit clause protects our developers from being bound to an irrational contract in the event that the price BTS:USD collapses. 

**SAFETY BUFFER**
Hourly estimates, given by the developers for the pre-defined list of BSIP's to be delivered, will have up to 15% additional buffer to be distributed by the worker manager. In this manner, both developers and shareholders are protected in terms of deliveries by providing additional budget, if needed, for any unexpected events.

**Payments/Escrow**
All employee hour reports will be done once every 3 weeks.  A single collective team invoice will be issued by the Worker Manager (Milos Preocanin) with previous approval by the Audit Manager (Abit More).  

Terms of Agreement
==================

- Milos Preocanin will be held as the principal responsible contact for this worker, on behalf of “Zavod Premik”.

- “Zavod Premik” will uphold this agreement with the BitShares Blockchain for 6 months, providing warranty on the declared structure and intent as being consistent to its purpose and fully transparent to the BTS token holders and BitShares Ecosystem.

- This project’s agreement is crafted as a middle path between the desires of BTS core token holders, priorities for the blockchain and technology, as well as availability of core development team. 

- Any additional requests from BTS token holders upon first activation of this worker will result in additional estimate and budget for integration and development (a new milestone worker).


## IMPORTANT NOTICE

This worker approves development of the listed BSIP's and other improvements, but not their inclusion within release of BitShares Core software as result of this worker. To make sure BSIP's are included in the upcoming release, BTS Core token holders must keep them actively voted in during last 90 days of this worker. 
